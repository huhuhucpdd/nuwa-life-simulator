...
- **量子人生模拟器（推演层）**  
  *一条靠谱的推演，必须知道自己什么时候开始不准。*  
  <br/>
  **⏳ 三条基础时间线**  
  `α线（基准）` 按你当前惯性自然延伸 · `β线（激进）` 把你不安全感压低30%，探索“更敢的自己” · `γ线（随机）` 注入黑天鹅事件，展示人生意外剧本。  
  <br/>
  **⚙️ 三种可选引擎**  
  - `混沌引擎`：纯随机上帝骰子，用于展示极端可能性空间  
  - `统计引擎`：基于真实社会统计学模型（职业转型成功率、地域流动性等群体数据）  
  - `混合引擎`（推荐）：**统计基线 + 你的个人偏离量 + 残余随机噪声** —— 既有普适规律，又保留你的独特决策指纹  
  <br/>
  **☢️ 半衰期预测引擎**  
  每条推演结果都自带“保质期”：  
  - `即时层（0-6个月）` 可信度 **70-85%**  
  - `短期层（6个月-2年）` 可信度 **50-65%**  
  - `中期层（2-5年）` 可信度 **30-45%**  
  - `混沌层（5年以上）` 可信度降至 **10-20%**，只展示主题风向  
  时间越远，我们越诚实。  
  <br/>
  **🕸️ 三层因果网络**  
